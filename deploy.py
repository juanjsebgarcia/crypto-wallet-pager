...
try:
    base_currency = json.loads(open('settings.json', 'r').read())[
        'Base_currency']
except:
...
for account in accounts:
    ...
    if account['wallet'] is not None or account['wallet'] != '':
        ...
        if eth_local_value > 0:
            ...
            wallet_string += "ETH: ~{} {}\n".format(
                tools.pretty_output(eth_local_value), account['currency'])
        ...
        for token in contracts:
            ...
            token_balance = web3.get_token_balance(
                token['decimal'], token_abi, account['wallet'])
            token_usd_value = forex.get_usd_value(
                token['symbol']) * token_balance
            token_local_value = forex.from_usd(
                account['currency'], token_usd_value)
            if token_local_value > 0:
                wallet_balance += token_local_value
                wallet_string += "{}: ~{} {}\n".format(
                    str(token['symbol']), tools.pretty_output(token_local_value), account['currency'])
        ...
        if 'bitcoin' in account:
            ...
            for address in bitcoin_addresses:
                ...
                bitcoin_local_value += forex.from_usd(
                    account['currency'], bitcoin_usd_value)
            if bitcoin_local_value > 0:
                wallet_balance += bitcoin_local_value
                wallet_string += "{}: ~{} {}\n".format(
                    'BTC', tools.prettyOutput(bitcoin_local_value), account['currency'])
    ...
    if EMAIL:
        if len(account['email']) > 5:
            if account['emailEnabled'] == "True":
                mail.mail_send(account['email'], header
                               + str(wallet_balance), content)
...
summary = content_total + \
    "Grand Total: {} {}".format(
        tools.pretty_output(wallets_total), base_currency)
print(summary)

# SEND SUMMARY
if EMAIL or SMS:
    try:
        admin_email = json.loads(open('settings.json', 'r').read())[
            'Email_FROMEMAIL']
        mail.mail_send(admin_email, 'Crypto {}'.format(
